...
//!
//! Default N = 1000 (CI-friendly). `FERROXY_IO_LARGE=1` raises to 10 000
...
use ferroxy_io::{serve, ServeSpec};
...
/// is 10 000 (`FERROXY_IO_LARGE=1`); see `phase1_deviations` in the
/// project memory — there is a scaling issue at high concurrency that
/// is not yet resolved. Override via `FERROXY_IO_N=<n>` for ad-hoc
/// experiments.
fn target_count() -> usize {
    if std::env::var("FERROXY_IO_LARGE").as_deref() == Ok("1") {
        10_000
    } else if let Ok(s) = std::env::var("FERROXY_IO_N") {
...
fn echo_thread_per_core() {
    ...
    let payload: Vec<u8> = b"ferroxy-phase-1-echo".to_vec();
...
fn no_listeners_rejected() {
    ...
    assert!(matches!(err, ferroxy_io::BindError::NoListeners));
}
